...
class AnnotationProcessor : AbstractProcessor() {
    ...
    companion object {
        ...
        private const val REPOSITORY_CLASS_NAME = "AppRepository"
    ...
    override fun process(p0: MutableSet<out TypeElement>?, roundEnvironment: RoundEnvironment?): Boolean {
        ...
        val mPresenter = ClassName(MVI_PACKAGE, PRESENTER_CLASS_NAME)
        val kPresenterClass = ClassName("kotlin.reflect", "KClass").parameterizedBy(
            WildcardTypeName.producerOf(
                mPresenter.parameterizedBy(WildcardTypeName.producerOf(mView))
            )
        )

        val presenterList = ClassName("kotlin.collections", "MutableList")
            .parameterizedBy(kPresenterClass)
        val mPresenterListHashMap = ParameterSpec.builder(
            "mPresenterListHashMap",
            HashMap::class.asTypeName().parameterizedBy(string, presenterList)
        ).build()
        ...
        mContractElement.forEach { contractElement ->
            val uiNames = contractElement.getAnnotation(Contract::class.java).uiNames
            uiNames.forEachIndexed { index, s ->
                funSpecBuilder.addCode("val mPresenterList$s = mutableListOf<KClass<out AppPresenter<out AppView>>>()\n")
                ...
                contractElement.enclosedElements.forEach { childElement ->
                    if (childElement.kind.isClass) {
                        ...
                        funSpecBuilder.addCode("mPresenterList$s.add(m$mPresenterName)\n")
                    }
                }
                funSpecBuilder.addCode("mPresenterListHashMap[\"$s\"]= mPresenterList$s\n")
            }
        }
